...
The general lesson is worth stating, because it is what costs the build cycle: **for AL property and operator eligibility, the compiler is the only authority.** Learn's `InitValue` page does not mention the type restriction, a Learn search for `AL0844` returns nothing, and Learn's own binary arithmetic operator table omits `Duration` altogether. These are silences, not permissions — and a search for `duration * number` returns Power Query M rules, which are a different language. Where the docs are silent on eligibility, prior art in the codebase and a compile are the evidence; a docs page that fails to forbid something is not a docs page that allows it.
...
- **Type not on the list** — assign the default on the record's creation path, in the slot between `Init()` and the `OnBefore…Insert` event. The ordering is the point, not just the location: the event receives the record by reference with the default already applied, so a subscriber can change it or set `IsHandled` and suppress the insert. Assigning *after* the `OnBefore` event silently overwrites whatever the subscriber chose. In the `OnBefore` → `Do` → `OnAfter` triple that `custom/knowledge/events/cmfrt-onbefore-do-onafter` prescribes, the assignment goes in the `Do` local, never in the public wrapper. A read-time getter is the wrong home in every case: it fires on every read and cannot distinguish an unconfigured field from a deliberate zero.
  Know what actually reaches that path. A singleton created from a setup page's `OnOpenPage` gets its default only when someone first opens that page — an app with no install codeunit has no guarantee the default runs before the feature does. Where an install codeunit exists it is the stronger home; where it does not, the gap is real and belongs in the recorded decision below rather than assumed away.
- **New field whose type blocks `InitValue`** — consider changing the type or unit instead. A `Duration` in milliseconds becomes an `Integer` in minutes with `InitValue = 15;`, which is legal, more readable on the setup page, and free of schema cost while the field is unshipped. The unit conversion then lives in one getter; converting a stored unit is not a hardcoded default and does not reoffend. This is usually cheaper than working around the restriction. Budget the name: a unit suffix competes with the `CMFRT <ABBR> ` prefix for the 30-character object-name limit — `"CMFRT AQ FS Stuck Threshold Min."` is 32 characters and does not compile, so the worked case ships as `"CMFRT AQ FS Stuck Thresh. Min."` at exactly 30, with the readable unit in the `Caption`, which has no such limit. See `custom/knowledge/naming/cmfrt-object-name-30-char-limit`.
...
`custom/knowledge/events/cmfrt-onbefore-do-onafter.md` owns the wrapper/`Do`/event triple this article's creation-path rule places the assignment inside; it is not contradicted here.
