...
Read the BCQuality knowledge index once (`knowledge-index.json` at the checkout root). Take every entry whose `layer` is `custom` as the candidate set — this covers all domains under `custom/knowledge/`: `naming`, `security`, `breaking-changes`, `events`, `patterns`, `architecture`, and `ui`. Do not open individual article files at this step.
...
- Namespace and import shape: a file declaring `namespace`, carrying `using` directives, or added as a new file to a project whose other files declare namespaces. Match against the `patterns` candidate `cmfrt-namespace-using-directives`. Evaluate the file's *whole* import block, not only the changed lines — a `using` list is correct or incorrect as a set, and the reference that lacks an import is usually not on the same line as the block. Because a review sees a diff rather than a compiled project, a missing or invented import is `confidence: medium` at best unless the referenced object's namespace was actually resolved from the symbol packages; report it as unverified rather than asserting the correct namespace.
